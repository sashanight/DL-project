...
class ESN(nn.Module):
    ...
    def esn(self, out, torev):
        hidden_states = torch.zeros(out.size()[1], out.size()[0], self.n_reservoir)
    ...
    def forward(self, batch, se_params):
        lengths, emb_fwd = model_utils.embed(batch, self.params, se_params)
        _, emb_rev = model_utils.embed(batch, self.params, se_params, to_reverse=1)
