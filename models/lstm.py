...
class RandLSTM(nn.Module):
    ...
    def lstm(self, inputs, lengths):
        ...
        e_hid_init = self.e_hid_init.expand(self.num_layers*self.bidirectional, bsz, self.output_dim).contiguous()
        e_cell_init = self.e_cell_init.expand(self.num_layers*self.bidirectional, bsz, self.output_dim).contiguous()
